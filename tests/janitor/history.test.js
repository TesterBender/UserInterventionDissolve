...
function idsFor(messages, history) {
  const ids = assignIdentities(messages);
  return history.map((entry) => ids[entry.index]);
}
...
describe('toStShape', () => {
  ...
  const shaped = toStShape(history, idsFor(PROVIDER_MESSAGES, history));
  ...
  it('produces the fields deriveFrontier reads and no invented name', () => {
    ...
    expect(typeof shaped[0].extra.userInterventionDissolve.id).toBe('string');
  });

  it('mutates no input message', () => {
    const before = JSON.stringify(history);
    toStShape(history, idsFor(PROVIDER_MESSAGES, history));
...
describe('the shim against unmodified src/', () => {
  const { history } = classifyMessages(PROVIDER_MESSAGES, ENVELOPE_CHAT_MESSAGES);
  const ids = idsFor(PROVIDER_MESSAGES, history);

  it('routes a user message through toManuscriptBlock by virtue of the shim alone', () => {
    const state = createState();
    const { text } = deriveFrontier(toStShape(history, ids), state, LITERAL);
  ...
  it('drops a message whose id is in frozenIds, with no special-casing in janitor/', () => {
    ...
    const shaped = toStShape(history, ids);
  ...
  it('round-trips to {role, content} pairs alternating assistant/user', () => {
    ...
    const frontier = deriveFrontier(toStShape(history, ids), state, LITERAL).text;
