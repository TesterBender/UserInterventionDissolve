...
4. If you are in a worktree under `.claude/worktrees/` and `node_modules` is missing, do NOT run `npm install`; create a junction to the main checkout's install with Node (no shell built-ins): `node -e "require('fs').symlinkSync('C:\\Users\\Asus\\OneDrive\\Desktop\\SillyTv\\UserInterventionDissolve\\node_modules', 'node_modules', 'junction')"`. If that is refused or fails, report `BLOCKED` per the rule below rather than trying another route.
