...
- `STATE_VERSION` is `2`, with a one-shot in-place `migrateV1` (`docs/modules/state.md#unknown-version`). Its approximation: a chat that already has frozen spans keeps its v1 frontier tail as an extra frozen span, so the model's view stays byte-continuous, but that tail becomes uneditable and a trailing incomplete block in it is dropped. Every existing non-system message is marked consumed, so nothing is sent twice.
...
Brief 0021 removed the bookkeeping this decision left as dead weight once derivation was in place: the `captured` message marker and the `MESSAGE_SENT` subscription that wrote it (nothing consulted it — derivation transforms every user message fresh, from its current text, on every request), the `received` message marker and the resample bypass that existed only to get past it (classification is now unconditional and idempotent), and `migrateV1` (a stored structure whose `version` is not `2` is now discarded and replaced with a fresh `createState()`, warned once, rather than migrated). Ids are now written by `recovery` alone, on the `saveChat()` it already performs; `assignIds`, `getState`, `createState` and the rest of the surviving state surface are unchanged.
