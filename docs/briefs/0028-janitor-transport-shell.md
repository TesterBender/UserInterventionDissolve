...
## Acceptance
- [x] With `transform` returning falsy, the fetch wrapper dispatches a body string `===` the caller's original for: the `/generateAlpha` envelope fixture, the chat-completions fixture, a POST to a non-matching URL, a non-POST request to a matching URL, and a POST of a non-JSON body. Asserted on the recorded `(resource, config)` of the fake `fetch`, by strict equality of the body string and by identity of the `resource` object where no re-serialisation occurred.
- [x] With a transform that mutates `data` and returns truthy, the dispatched body is `JSON.stringify(data)` and differs from the original; with a transform that mutates nothing and returns falsy, the original string is dispatched even though `JSON.parse`/`stringify` would have reordered nothing.
- [x] Response bytes read by the caller through the wrapper equal the upstream chunks concatenated, in order, and `status`, `statusText`, `headers.get(...)` and `url` survive the wrap; cancelling the wrapped body cancels upstream.
- [x] `readEnvelope` on the fixture returns `personaName` from `profile.name` (a test fixture where `profile.user_name` differs proves `user_name` is never used), the `chatId`, `generateType`, prefill settings, the parsed route, and `chatMessages` positions with `isMain` flags.
- [x] `isTargetedCompletion` returns false for the alpha URL, false for a completion-shaped URL on a different host once a route is bound, true for the bound route's URL, and — after 5 s of simulated time with no conversation-identity match — false via the expired bridge (fake timers).
- [x] A non-native `window.fetch` at install time produces exactly one `console.warn` and still installs.
- [x] `installXhrWarning` warns once on a completion-shaped `open()` and calls through; no test asserts any XHR body or response behaviour, because none exists.
- [x] No file under `janitor/` imports from `src/` or references `SillyTavern`.
- [x] `npm run check` passes (`check:comments` now covers `janitor`).
- [x] every new pointer comment resolves (`node tools/check-comments.mjs`)

Two notes on how the boxes were met. **Expired bridge**: after 5 s the test asserts `currentEnvelope()` is `null` and `routeMatches(bound URL)` is `false`; `isTargetedCompletion` itself then returns `true` for a completion-shaped URL, because with no route bound it falls back to `COMPLETION_URL_PATTERN` exactly as the cited optimizer rule (L2195–2203) prescribes. Both behaviours are asserted. **Native fetch**: only the non-native branch is testable — `Function.prototype.toString.call` cannot be made to report `[native code]` for a test double — so no test covers the silent install.
