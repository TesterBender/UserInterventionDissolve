...
Orchestrator clarifications after the scope audit; no code change.

1. **Prefill.** Step 6 appends every injection to the first system message *except* a trailing `assistant`-role injection, which step 9 drops instead. Janitor's prefill has no envelope counterpart, so the diff of step 3b classifies it as an injection; folding it would put the prefill text into the system message. Dropping it is what `TamperContainment/PLAN-janitor.md` ("Strip Janitor's trailing assistant prefill") settles. The test is positional and role-based; no Janitor setting is read.
2. **Report.** The `console.info` line carries five facts, the fifth being the `classifyDrift` count that step 5 computes "for the report"; this line is the only report until the panel exists.
3. **Lead-in absent case.** The acceptance test for a reconstruction without the lead-in is exercised where no assistant turn follows the system message; a reconstruction opening on a `user` turn is unreachable because `buildHistory` always opens on an assistant span, and no branch exists for it.
4. **Docs.** `docs/modules/janitor-adapter.md` may also replace its phase-3 "nothing here is wired" paragraph, which this brief makes false.
