...
Depends on: brief 0032 (envelope diff, identity, shim — merged) and brief 0033 (request transform — merged). This brief amends what both produced; it does not restate their scope.

Scope source: `TamperContainment/PLAN-janitor.md` — "State and identity on Janitor" (envelope ids positional and used only for alignment; identity is `fnv1a32(role + raw content)` plus occurrence index) and "Per-generation pipeline" request-side steps 3b (diff the provider `messages` against the envelope's `chatMessages`; a message with no counterpart is an injection) and 5 (derive: `deriveFrontier` turns user turns into `Persona:\n…` blocks). Host: janitorai.com only; `docs/protocol/host-mapping.md` does not apply.
...
**`janitor/history.js`**
- `classifyMessages(messages, envelopeChatMessages, personaName)` gains a third parameter, the persona name from the envelope's `profile.name` (`docs/api/janitor.md#persona-name-lives-at-profile-name`) — the same value the call site already turns into the reserved literal. Absent or empty, the function behaves exactly as it does today.
...
**`janitor/transform.js`** — the call site only: pass `context.personaName` as the third argument to `classifyMessages`. Nothing else in the pipeline changes, and the step order of `docs/modules/janitor-adapter.md#request-pipeline` is untouched.

**`janitor/identity.js`** — expected to need **no edit**. It is on the allowlist only in case threading the bare text requires one; if no edit is needed, leave it byte-identical.
