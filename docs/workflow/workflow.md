...
2. The orchestrator does the swap itself with a script when the change is a verbatim text replacement: write the new text to a scratch file, replace the template literal in the constant's file and in the test's expected copy (escaping backticks), update the word-count freeze, regenerate `presets/`, run the check. Only when the change touches logic (assembly order, sanitiser, new exports) does a Sonnet `implementer` run **in place** with the amendment as its brief. Either way: the constant, the assertions that pin it, and the doc heading that quotes it. Nothing else.
