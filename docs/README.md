...
| `modules/` | One file per `src/` module: what it does, its invariants, edge cases. Targets of most pointer comments. `janitor-transport.md` covers the `janitor/` fetch shell instead, and `janitor-build.md` the userscript bundler (`tools/build-janitor.mjs`). | implementer |
