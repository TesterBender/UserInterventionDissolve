...
## Addendum: hierarchical compilation (2026-09-13) {#hierarchical-compilation}
`PLAN-addendum-hierarchical-compilation.md` adds internal compilation tiers and a cache-stable final shape. Consequences for the invariants: INV-4/INV-5: the model-visible history is `[final span, continuation] × n` followed by `[current frontier, continuation]`, where the current frontier is ONE assistant message made of not-yet-final compiled units plus the hot derived text (addendum §8, §10). INV-6: the ~3,000–4,200-word cut produces a *compiled unit*, not a final span; units are promoted into final spans bounded by a ≈10,000-word ceiling, and a final span plus its following canonical continuation turn are append-only and never merged, reordered or reworded (§7, §11). INV-7: unchanged. INV-10: internal tiers never appear as chat topology.
