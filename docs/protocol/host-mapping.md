...
- `generate_interceptor` receives a per-request copy of the non-system messages for **both** chat- and text-completion paths and mutations affect only that request (`#generate-interceptor`). `frontier` empties it and pushes the tiered shape (`docs/modules/frontier.md#shape`): for each **final** frozen span an assistant message followed by one byte-identical canonical continuation user turn, then one assistant message carrying the unsealed Tier-1 units plus the derived frontier joined with the block delimiter, then the single live edge user turn. Internal tier boundaries add no turns, and the `[final, control]` prefix is emitted from canonical state alone, so it grows monotonically and is byte-identical across calls that change only the frontier (`PLAN-addendum-hierarchical-compilation.md` §5–§8, §11).
...
- Compilation is two-tier (`PLAN-addendum-hierarchical-compilation.md`, `docs/decisions/0006-hierarchical-compilation.md`). `selectCut` is pure and unchanged: given the frontier text and a target (3,000–4,200 words with jitter, a host constant — the one place PLAN names a configurable), it picks the cut per §17 salience rules and decides nothing else. `compileUnit` is the apply step and does mutate canonical state — `pushUnit` appends a **Tier-1 unit**, `advanceWatermark` records what it consumed (`docs/modules/state.md#push-unit`, `#advance-watermark`) — and `sealUnits` promotes the accumulated units into one **final** frozen span once the next unit could no longer fit under the 10,000-word ceiling (`docs/modules/freeze.md#seal-policy`). `recovery` is what persists the result with `saveMetadata` (`#chat-metadata`), once per receipt and only when a unit was actually pushed.
- **Compiled once, remembered — of units and final spans.** Compiled text is compiled exactly once and persisted; later requests read it back verbatim and never recompile it. That covers Tier-1 units as well as the final spans they are sealed into: a unit's text is already off `chat[]` and its messages are already in `frozenIds`, and sealing only concatenates units that already exist. Final spans are append-only in the strong sense — never merged, re-cut or rewritten once created (addendum §7, §11; INV-6). The `chat[]` messages it was compiled from are no longer inputs: their ids are listed in `frozenIds` and the derivation skips them, so editing, deleting or swiping *those* messages after the freeze changes the visible log only. The **uncompiled** region is the opposite and always was: nothing has been compiled there, so there is nothing to remember, and it is re-read from `chat[]` on every request (`docs/modules/derive.md#derivation-rule`). Authorial editing (§10) therefore acts on the uncompiled region alone, and an edit there is picked up by the next request without touching any frozen span. (User's framing, 2026-09-12: a compile "is something that is remembered, such that previous message structures, after being compiled, no longer need to be recompiled again".)
...
- **Recompile.** A deliberate whole-chat recompile (`/uidrecompile` or the drawer button, `docs/modules/recompile.md#what-it-is`) discards the stored cut bookkeeping and rebuilds it from `chat[]` under the rules currently in the code; it is collaborator-initiated only and re-cuts nothing that survives, because nothing survives the reset.
