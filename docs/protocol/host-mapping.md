...
- `selectCut` is pure: given the frontier text and a target (3,000–4,200 words with jitter, a host constant — the one place PLAN names a configurable), it picks the cut per §17 salience rules and decides nothing else. `maybeFreeze` is the apply step and does mutate canonical state — `pushFrozen` appends the span, `advanceWatermark` records what it consumed (`docs/modules/state.md#append-only`, `#advance-watermark`) — and `recovery` is what persists the result with `saveMetadata` (`#chat-metadata`), once per receipt and only when a span was actually pushed.
