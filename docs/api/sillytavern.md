...
notes: Returns a cleaned string (or extracted JSON string if `jsonSchema` set, via `generateRawData`). Uses `main_api`'s **connection settings** (temperature, model, preset — read by `sendOpenAIRequest`/`getKoboldGenerationData`/etc. from `oai_settings`/`kai_settings`) but the prompt/system-prompt content is **entirely replaced** by the args — no character card, WI, persona, or chat history is injected (`createRawPrompt`, `:3865-3904`). **Corrected 2026-09-12** (previous version of this entry wrongly claimed only `GENERATE_AFTER_COMBINE_PROMPTS`/`CHAT_COMPLETION_PROMPT_READY` fire; live evidence from a user's 1.18.0 run showed a custom stop string injected from a `CHAT_COMPLETION_SETTINGS_READY` listener cutting off a `generateRaw` call, so that event does fire on this path). Full ordered event list, chat-completion path: `CHAT_COMPLETION_PROMPT_READY` (`script.js:3976-3980`, array-form payload `{chat, dryRun:false}`, in-place mutation honored, replacement not — same as #chat-completion-prompt-ready) → `CHAT_COMPLETION_SETTINGS_READY` (`generateData` = the chat array itself passed to `sendOpenAIRequest('quiet', …)`, which emits it at `openai.js:3052` before the fetch — `generateData` is mutable and read straight into the request body, so a listener-set `stop` array reaches the backend) → fetch. Text-completion path: `GENERATE_AFTER_COMBINE_PROMPTS` (`script.js:3970-3973`, `{prompt, dryRun:false}`, string form) → `getTextGenGenerationData` → `TEXT_COMPLETION_SETTINGS_READY` (`textgen-settings.js:1847`, `params` mutable, emitted just before its own fetch) → fetch. Neither path streams: `sendOpenAIRequest` type is `'quiet'`, and `stream = settings.stream_openai && type !== 'quiet' && …` (`openai.js:2724`) is always `false` for `generateRaw`/`generateRawData`, so `STREAM_TOKEN_RECEIVED` cannot fire; text-completion generation functions likewise take a `'quiet'` type with no streaming branch exercised here. `GENERATION_STARTED`/`GENERATION_AFTER_COMMANDS`/`MESSAGE_RECEIVED` do **not** fire — none of those emits appear anywhere in `generateRawData`/`generateRaw` (`script.js:3938-4062`), only inside `Generate()` (`:4240` onward). `runGenerationInterceptors` (`generate_interceptor`) is also never called for `generateRaw`. This is the right call for "rewrite this text under this system prompt, nothing else in context," but extensions that inject stop strings or other settings via `CHAT_COMPLETION_SETTINGS_READY`/`TEXT_COMPLETION_SETTINGS_READY` globally will affect `generateRaw` calls too — there is no `generateRaw`-only exemption.
...
### Custom prompt-manager entry {#preset-custom-entry}
status: verified
checked: 1.18.0 @ 8172dcd on 2026-09-13
evidence: `public/scripts/PromptManager.js:988-999` — `addPrompt(prompt, identifier){ const newPrompt = { identifier, system_prompt:false, enabled:false, marker:false, ...prompt }; this.serviceSettings.prompts.push(newPrompt); }`; `public/scripts/openai.js:1227-1237` — `const userRelativePrompts = prompts.collection.filter((p)=>false===p.system_prompt && p.injection_position!==INJECTION_POSITION.ABSOLUTE)...`; `public/scripts/PromptManager.js:264,274,3908` — `index(id){ return this.collection.findIndex(p=>p.identifier===id); }`, `add(collection, position){ this.messages.collection[position] = collection; }`.
notes: (1) Minimal entry: `{identifier, name, role, content}` in `prompts[]` — `system_prompt`/`marker`/`enabled` default `false` via `addPrompt` spread if omitted; `injection_position` unset ⇒ `undefined !== INJECTION_POSITION.ABSOLUTE(1)` is true, so it's treated as **relative** (in `prompt_order`) by default, not absolute-in-chat. `injection_position:0`=relative (goes through `prompts.index()`/`prompt_order` rank), `1`=absolute (injected at `injection_depth` via `populationInjectionPrompts`, `openai.js:1189-1240`, bypassing `prompt_order` rank entirely). `injection_depth`/`injection_order`/`forbid_overrides` only matter for absolute prompts or overriding `main`/`nsfw`/`jailbreak`; no import/render error if left unset for a plain relative custom entry. No `isPromptInjectable` function exists in this codebase (grep empty) — that name doesn't exist; use `injection_position`.
(2) Yes. `PromptManager.getPromptCollection` (`PromptManager.js:1516-1541`) builds the ordered `PromptCollection` by walking `prompt_order[].order` in array order (enabled entries only), so `prompts.index(id)` reflects `prompt_order` rank. `populateChatCompletion`'s `addToChatCompletion` (`openai.js:868-889`) does `chatCompletion.add(collection, prompts.index(source))`, and `ChatCompletion.add` (`openai.js:3903-3908`) does `this.messages.collection[position] = collection` — a direct array-index write. `chatHistory` itself is placed the same way (`openai.js:881` `chatCompletion.add(new MessageCollection('chatHistory'), prompts.index('chatHistory'))`) and then filled in-place by `populateChatHistory`. So a relative custom entry ranked after `chatHistory` in `prompt_order` occupies a higher final array index and its message renders after every chat-history message, immediately before generation.
(3) Confirmed. `getPromptCollection` (`PromptManager.js:1521-1537`): `const allowedTrigger = entry.enabled && this.shouldTrigger(...)`; if not `allowedTrigger` and not `'main'`, the prompt is skipped entirely (not pushed into the collection), so it never reaches the request. The `enabled` flag lives only on the `prompt_order` entry object (not on the `prompts[]` object itself), which is exactly what the prompt-manager popup checkbox toggles (`PromptManager.js:523-539` reads/writes `prompt.enabled`/order entry) — independent of request inclusion, so it stays visible/toggleable in the UI while excluded from the request.
(4) `role` is read straight through by `Message.fromPromptAsync`→`Message.createAsync(role, content, identifier)` (`PromptManager.js` `Prompt` ctor / `openai.js:3464`) with no allowed-value validation in this file, but system/user/assistant are the only roles ST's own default/nsfw/jailbreak/impersonate entries and the OpenAI/Claude message schemas use — no other role is handled elsewhere in `openai.js`. Placement caveat: since `chatHistory` is a single named `MessageCollection` slot in `prompt_order` rank order and our continuation user turn is injected *into* that `chatHistory` collection (via `chatCompletion.insert(message, 'chatHistory', …)`, not appended after it as a separate ranked slot), a custom entry ranked after `chatHistory` — system or user role — lands after our injected user turn too, not just after the pre-existing history. `role:'system'` is the sane choice for a final instruction there: most providers otherwise expect the last message to be `user` (some backends error/warn on trailing `system`/`assistant` and Claude requires the transcript end in `user` for the API, though ST does not enforce this locally) — a checked-in decision should note the backend-specific tail-role constraint if using `system` last.
notes-extra: `forbid_overrides:false` and `injection_order:100` (`DEFAULT_ORDER`, `PromptManager.js:31`) match the constructor defaults (`PromptManager.js:182-196`) if the field is present at all; omitting them is fine, `?? DEFAULT_ORDER`/`?? DEFAULT_DEPTH(4)` cover unset values only where the value is read at edit-time in the popup, not at request-assembly time for relative prompts (irrelevant there).
