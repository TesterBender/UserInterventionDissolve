...
PLAN §11's "block merged into the manuscript" no longer copies anything: the received message stays in `chat[]` and derivation reads it there on the next request. What happens at receipt is therefore only what must happen *before* that read — classification, the in-message rollback, the in-message boundary trim, and one marker.

The `received` marker (v1 called it `appended`) lives in `message.extra[METADATA_KEY]` and is written with a spread over whatever is already there, so sibling flags owned by other modules in the same namespace survive (`boundary` writes `boundary`, `capture` writes `captured`, `derive` writes `id`). It is a **message-local idempotence flag, not canonical state**: it records that this chat entry has already been through the classifier, so a repeated MESSAGE_RECEIVED for the same index cannot classify twice. No text is recorded alongside it — there is no `appendedText`, because there is no second copy whose drift would need detecting.
...
The one thing the resample case still needs is to get *past* the idempotence guard: the `received` marker is already on the message, and the new text may itself be an incomplete trailing block that INV-8 requires rolling back. So `type === 'swipe' || type === 'regenerate'` bypasses the marker check and the message is classified again. That is the only remaining use of the type on this path.
