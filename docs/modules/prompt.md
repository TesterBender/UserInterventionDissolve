...
The system prompt does two things at once, and the order matters. It opens as craft framing — creative writing shaped by what already stands on the page, written as prose that is particular, sensory, willing to be strange — and only then lists a few formatting rules. PLAN §20 is explicit that "the grammar exists to preserve agency, not to flatten prose into legalese": a bare rule list gets obeyed as a form, producing stage directions rather than fiction. The framing is what keeps the rules in service of the writing.

What the rule list commits to is exactly the load-bearing semantics and nothing more (§5–§7):

- blocks are separated by a blank line, and each block is tagged or it is narration;
- a tagged block belongs to one figure alone, and that ownership covers interiority as well as action — speech, action, choice, intent, attention, private interpretation;
- narration carries the world rather than the will: it integrates what was already chosen and decides for nobody ("tags apply impulses, buffers integrate them");
- a group tag stands only for those still anonymous within it, so an individually tagged figure is never swept into an aggregate (§7, INV-9).

Two further lines carry §6 and the anti-closure habit: a tagged intention holds until something ends it and later narration may carry it forward, and "the story closes when the story does."
