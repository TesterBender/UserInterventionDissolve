...
`content` is the string the [envelope diff](#envelope-diff) emitted, which for a persona-prefixed user turn is the **bare** envelope text, not the transport string. The stored identity must not depend on a decoration Janitor owns: if it did, the next Janitor build that changed or dropped the `Name: ` prefix would silently re-key every compiled user turn, and compiled manuscript would be re-sent as fresh frontier. The same reasoning applies to the watermark's `prefixHash` and to `watermarkText` — both describe a slice of the derived manuscript, not of Janitor's transport string.
...
- **The envelope's own ids / positions.** The 2026-09-13 claim that these ids are positional was wrong: they are large integer database ids (`docs/api/janitor.md#envelope-message-entries-carry-database-ids`). Whether they would serve as stored identity is therefore an *open* question for a follow-up brief, not a settled rejection. Either way nothing in this layer stores or reads them; the envelope is used for *alignment within one request* only (see [Envelope diff](#envelope-diff)).
...
- **Alignment is by content, not by the envelope's ids.** Those ids are database ids whose relation to a chat's ordering is uncaptured (see [Content-hash identity](#content-hash-identity)); the order of the conversation is the thing both sides agree on.
...
The history entry carries the bare text because Janitor added the prefix and the human did not: everything downstream — the sentinel test, `assignIdentities`, `matchWatermark`, `toStShape`, `deriveFrontier` and `toManuscriptBlock` — must see exactly what the collaborator wrote, so the derivation writes the own-line `Persona:` header itself, as it does on SillyTavern. Without this the prefixed turn matched nothing, was classified as an injection and was folded into the system message, and the human's contribution never reached the frontier at all (INV-3).
