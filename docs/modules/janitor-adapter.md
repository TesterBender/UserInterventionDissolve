...
The message is composed as `[manuscript prompt] + [the editable unit] + [folded injections]`. The **editable unit** is the saved context override when this chat has one ([Context override](#context-override)) and Janitor's own captured first system message otherwise. The capture is read from the *incoming* message array, before the transform reassigns it, so it is Janitor's text and only that: the prompt is prepended after it and the injections are appended after it, and neither can ever be part of the unit. The first-sentence probe above applies to whichever text occupies the unit — an override that itself carries the prompt's first sentence suppresses the prepend exactly as Janitor's own text would, and there is no special case for that. Injections are still appended last, which is what keeps the cached prefix stable and what makes the override's change detection immune to them.
...
A human may replace the editable unit for one chat. The override is the whole first system message as a single blob, and it is precisely three things it is not: it is not the manuscript prompt (prepended afterwards, never stored, never edited here), not the folded injections (appended afterwards, excluded from the unit by construction rather than by a filter), and not a seam-matched split of Janitor's text into "your custom prompt" and "Janitor's hidden context".

The bundle model rather than a split, because the optimizer's own seam matcher degrades to the bundle whenever it fails — no `/generateAlpha` baseline, an empty hidden remainder, a prompt Janitor reflowed (`TamperContainment/TamperMonkeyJanAI.txt` L4482–4538). A matcher that silently falls back cannot be relied on, and on this host a mis-split would either rewrite Janitor's text in place, which [System message](#system-message) forbids, or hide part of the context from the human editing it.

`janitor/context-override.js` owns one `localStorage` entry per chat, keyed `JANITOR_OVERRIDE_KEY_PREFIX + chatId` — `uid-janitor-context-v1:<chat id>`. That prefix is deliberately not a `STORAGE_KEY_PREFIX` key and is not matched by a `startsWith(STORAGE_KEY_PREFIX)` scan, so a pass over the stored manuscripts never sees it. The stored value is `{ janitorOverrideFormat, text, capturedText, savedAt }`, where `capturedText` is a copy of the unit the override was saved against.

It is **its own entry with its own format version**, not a field of the stored state, for a decisive reason: a recompile replaces the whole state with `freshState()` ([Recompile](#recompile)), which would silently delete text a human wrote. Separating them also means a future override-shape bump discards no compiled manuscript and a `JANITOR_STATE_FORMAT` bump discards no override. `janitorOverrideFormat` is checked the way `janitorFormat` is ([Stored state](#stored-state)): **no migration and no repair**. A value that does not parse, carries another format, or has a non-string `text` is refused whole with one `console.warn`; a missing entry is not a warning. A write that throws is caught and warned and never raised at the caller, for the reason [Stored state](#stored-state) gives.

An **empty override is not an override**. `saveOverride` with text that is empty after `trim()` removes the entry instead of writing one, so the human's "go back to Janitor's text" and an emptied editor arrive at the same place — which is also what Clear does.

**Drift** is `overrideDrift(override, capturedText)`: true when an override exists and its `capturedText` differs from the current capture by exact string inequality — no trimming, no whitespace folding, no length heuristic. It is informational. Nothing blocks, clears, re-saves or re-prompts on it; Janitor changing its own context is a sentence for the human to read, not a reason to touch a request in flight. Because injections are outside the unit, adding, removing or rewording one never reports drift — which is exactly the optimizer's failure mode this design removes (`docs/decisions/0007-janitor-host-deviations.md`).

The override travels with [State export and import](#state-transfer): `exportStateJson` takes it as a third argument and writes it after `state`, and `importStateJson` returns it as `override`, `null` when the file has no `override` key (every export written before it existed) or when the one it has is malformed or carries another format. A malformed or absent override never refuses the import and never adds a reason: the manuscript is what the file exists to carry, and a state that imports while its override is rejected is better than losing both. Import **replaces the override wholesale, `null` included** — a file that restores a manuscript while leaving the previous chat's context in place is the merge [State export and import](#state-transfer) already refuses.

The override text is **user content**. The script never reads it, parses it, lints it, substitutes macros in it or rewords it; `docs/decisions/0003-duplication-filter-wording.md` constrains strings this repository pins and does not reach text a human wrote. Nothing in `src/prompt.js` changes.

INV-10 holds trivially ([INV-10 under envelope ids](#inv-10)): the override changes the content of one system message, which is not history. Its existence, its drift and its storage key reach neither the reconstructed history nor the request report, and the model sees one system message whose composition order is fixed whether an override is saved or not.
...
`janitor/status.js` holds one module-level object describing the last transformed request: `chatId`, `literal`, `finals`, `units`, `frontierWords`, `froze`, `rebuilt`, `stopSent`, `routerEnabled`, `driftNotices`, `capturedContext` (Janitor's own first system message for that request, so the panel can show what an override would replace; see [Context override](#context-override)) and `at` (epoch ms of the write). It is written once per transformed request, beside the `console.info` of [Request report](#request-report) and carrying the same facts, and it is read by the panel. Nothing in it reaches the model (§27). `getRequestStatus()` hands back a copy, so a renderer cannot write into the module's object.
