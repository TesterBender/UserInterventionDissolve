...
## Reserved spans {#reserved-spans}

`groupSpans(blocks, { reservedActor })` takes one optional argument: the name of the externally owned character, as the caller already computes it from the reserved literal (`docs/modules/boundary.md#reserved-literal`). A block whose start carries that actor's literal — tested with `findTagLiteral(block.raw, reservedActor)` and an `atBlockStart` hit, never with `TAG_HEADER` and never by comparing a parsed actor string — **always opens a span**, and that span is marked `reserved: true`.

This exists because the two tests do not coincide. `TAG_HEADER` deliberately restricts the characters a name may contain, so a persona called `Anton_S` or one whose name runs past forty characters produces a block that parses as `kind: 'buffer'`. Without this rule that block would *join the preceding span*, the preceding span would not be reserved, and `freeze` would be free to cut immediately before the external character — the exact thing INV-7 forbids (`docs/modules/freeze.md#salience-heuristics`). Ownership protection must not depend on whether the collaborator's persona name happens to be spellable by the tag pattern, which is the same rule `boundary` already follows for the stop string.

`reserved` is `false` on every span when no `reservedActor` is supplied, when it is empty, or when the literal is a bare `':'` — the ordinary case in which nothing is reserved. A block can be both a parsed tag header and a reserved opener; it opens one span, marked `reserved: true`, with `header` set to the reserved actor as the caller named it.
