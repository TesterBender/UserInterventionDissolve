...
- `prompts` — two entries. The `main` prompt carries the system prompt verbatim: `{identifier, name, role, content}` plus `system_prompt: true` is the verified minimum for a plain text prompt (`docs/api/sillytavern.md#preset-openai-prompt-entry-shape`). The second, `uidTakeStock`, is a custom entry: `{identifier, name, role, content}` with no `system_prompt`, `enabled`, or `marker` field, which is the verified minimum for a plain relative custom entry — those three default via `addPrompt`'s spread if omitted (`docs/api/sillytavern.md#preset-custom-entry`).
- `prompt_order` — one `{character_id, order}` list, now 13 entries: `main, uidTakeStock, worldInfoBefore, personaDescription, charDescription, charPersonality, scenario, enhanceDefinitions, nsfw, worldInfoAfter, dialogueExamples, chatHistory, jailbreak`. `character_id` is the number `100001`, the `dummyId` of the live openai `promptManagerModule`. A preset shipping only `100000` (the base `PromptManager` default, also present in ST's own `Default.json`) imports without error and then renders an **empty** prompt order — the prompt silently does nothing (same anchor).

`uidTakeStock` sits at `order[1]`, immediately after `main`, and ships `enabled: false`. Leaving `injection_position` unset keeps the entry relative — ordered by `prompt_order` rank rather than injected at a fixed depth (`docs/api/sillytavern.md#preset-custom-entry`) — and a relative entry ranked after `chatHistory` would land after every history message, including the injected continuation user turn: that would both displace the continuation seam from the edge of the request (INV-5) and leave the transcript ending on a `system`-role message, which some backends reject or warn on for a trailing turn. It is standing guidance rather than an edge instruction, so it belongs beside `main`, not near the active edge. `enabled: false` is explicit rather than omitted, because omitting an identifier from `order` does not disable it (see [Default order and markers](#default-order-and-markers)) — only an explicit `enabled: false` on the order entry keeps it out of the request while it stays visible and toggleable in the prompt manager's own list.
